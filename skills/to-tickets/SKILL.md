...
Publish the approved tickets. **How** depends on the tracker `/setup-engineering-workflow` configured — the tickets are the same either way, only the shape of the blocking edges changes:
...
Work the frontier one ticket at a time with `/deliver-ticket`, clearing context between tickets.
</content>
